        ...
        if ((strlen($login) > 0) AND ( strlen($password) > 0)) {
            ...
            if ($user) { // login ok
                if ($password == $user->password) { // password ok
                    if ($user->status != 0 && $user->is_new_user != 1) {  // success
                        $sections = $this->ci->user_type_model->get_user_type_permissions_by_section($user->member_type_id);
                        $modules = $this->ci->user_type_model->get_user_type_permissions_by_module($user->member_type_id);
